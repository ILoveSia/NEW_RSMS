 ...
 * - 책무상세 엑셀 업로드 양식 구조
 */
interface ResponsibilityDetailExcelData {
  책무코드: string;
  책무세부내용: string;
  사용여부: string;
}

interface ResponsibilityDetailExcelUploadModalProps {
  open: boolean;
  onClose: () => void;
  onUpload: (data: ResponsibilityDetailExcelData[]) => Promise<void>;
  loading?: boolean;
}

const ResponsibilityDetailExcelUploadModal: React.FC<ResponsibilityDetailExcelUploadModalProps> = ({
  open,
  onClose,
  onUpload,
  loading = false
  ...
  const [previewData, setPreviewData] = useState<ResponsibilityDetailExcelData[]>([]);
   ...
   * 미리보기 Grid 컬럼 정의
   */
  const previewColumns: ColDef<ResponsibilityDetailExcelData>[] = [
    { headerName: '책무코드', field: '책무코드', width: 150, sortable: true },
    { headerName: '책무세부내용', field: '책무세부내용', flex: 1, sortable: true, minWidth: 400 },
   ...
   * - 컬럼명 검증 및 데이터 변환
   */
  const parseExcelFile = useCallback(async (file: File): Promise<ResponsibilityDetailExcelData[]> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const requiredColumns = ['책무코드', '책무세부내용', '사용여부'];
          ...
          const parsedData: ResponsibilityDetailExcelData[] = dataRows
            .filter((row: any) => {
            ...
            .map((row: any, index: number) => {
              ...
              // 필수 필드 검증
              if (!rowData['책무코드'] || !rowData['책무세부내용']) {
                throw new Error(`${index + 2}행: 책무코드, 책무세부내용은 필수입니다.`);
              }
              ...
              return rowData as ResponsibilityDetailExcelData;
            });
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <Table size="small" sx={{ tableLayout: 'fixed', width: '100%', borderCollapse: 'collapse' }}>
                  <TableHead>
                    <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                      <TableCell sx={{ fontWeight: 600, fontSize: '0.75rem', width: '20%', padding: '8px', border: '1px solid #e0e0e0' }}>책무코드</TableCell>
                      <TableCell sx={{ fontWeight: 600, fontSize: '0.75rem', width: '65%', padding: '8px', border: '1px solid #e0e0e0' }}>책무세부내용</TableCell>
                      <TableCell sx={{ fontWeight: 600, fontSize: '0.75rem', width: '15%', padding: '8px', textAlign: 'center', border: '1px solid #e0e0e0' }}>사용여부</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    <TableRow>
                      <TableCell sx={{ fontSize: '0.7rem', color: 'text.secondary', padding: '8px', border: '1px solid #e0e0e0' }}>20250001M0001</TableCell>
                      <TableCell sx={{ fontSize: '0.7rem', color: 'text.secondary', padding: '8px', border: '1px solid #e0e0e0' }}>세부내용 0001-1 경영전략 업무와 관련된 책무</TableCell>
          ...
          <Alert severity="warning" icon={<WarningIcon />} sx={{ mt: 1 }}>
            ...
            <Typography variant="caption" component="ul" sx={{ m: 0, pl: 2 }}>
              ...
              <li>책무코드는 responsibilities 테이블에 존재하는 코드여야 합니다.</li>
...
export default ResponsibilityDetailExcelUploadModal;
