...
export const getEmployeeByEmpNo = async (empNo: string): Promise<Employee> => {
  try {
    const response = await apiClient.get<Employee>(`/employees/${empNo}`);
    return response.data;
  } catch (error) {
...
export const getEmployeesByOrgCode = async (orgCode: string): Promise<Employee[]> => {
  try {
    const response = await apiClient.get<Employee[]>(`/employees/org/${orgCode}`);
    return response.data;
  } catch (error) {
...
/**
 * 직급별 직원 목록 조회 API
 * - GET /employees/job-grade/{jobGrade}
 * - 특정 직급의 직원 목록 조회
 *
 * @param jobGrade 직급
 * @returns 직원 목록
...
export const getEmployeesByJobGrade = async (jobGrade: string): Promise<Employee[]> => {
  try {
    const response = await apiClient.get<Employee[]>(`/employees/job-grade/${jobGrade}`);
    return response.data;
  } catch (error) {
