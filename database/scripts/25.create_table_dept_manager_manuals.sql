...
-- 작성자: Claude AI
-- 작성일: 2025-11-07
-- 수정일: 2025-01-18 - 테이블 구조 변경 (manual_id → manual_cd, 컬럼 정리)
-- 참고:
--   - management_obligations 테이블과 N:1 관계 (obligation_cd FK)
--   - ledger_order 테이블과 N:1 관계 (ledger_order_id FK)
--   - organizations 테이블과 N:1 관계 (org_code FK)
--   - common_code_details 테이블의 FLFL_ISPC_FRCD 그룹코드 참조 (수행점검주기)
--   - manual_cd 코드 생성 규칙: obligation_cd + "A" + 순번(4자리)
...
BEGIN
  ...
  RAISE NOTICE '📋 기본키: manual_cd (VARCHAR(50))';
  RAISE NOTICE '📋 외래키: ledger_order_id, obligation_cd, org_code';
  RAISE NOTICE '📋 관계: management_obligations (N:1)';
  RAISE NOTICE '📋 코드 생성 규칙: obligation_cd + "A" + 순번(4자리)';
  RAISE NOTICE '📋 인덱스: 7개 생성 완료';
END $$;
