-- =====================================================================================
-- V012: 이행점검결과보고서 테이블 생성 (impl_inspection_reports)
-- =====================================================================================
-- 설명:
--   - 이행점검결과보고서 정보를 관리하는 테이블
--   - 이행점검결과보고서ID 코드 생성 규칙: impl_inspection_plan_id + "R" + 순번(3자리)
--     예: 20250001A0001R001
-- 작성일: 2025-11-26
-- Flyway 마이그레이션: V012
-- 참조: database/scripts/28.create_table_impl_inspection_reports.sql
-- =====================================================================================


-- =====================================================
-- 1. 이행점검결과보고서ID 자동 생성 함수
-- =====================================================
...
-- =====================================================
-- 2. impl_inspection_reports 테이블 생성
-- =====================================================
...
CREATE TABLE rsms.impl_inspection_reports (
  ...
  -- 기본 정보
  report_type_cd VARCHAR(20) NOT NULL,                    -- 보고서구분 (01:CEO보고서, 02:임원보고서)
...
CREATE INDEX idx_impl_inspection_reports_ledger_order ON rsms.impl_inspection_reports(ledger_order_id);
CREATE INDEX idx_impl_inspection_reports_plan ON rsms.impl_inspection_reports(impl_inspection_plan_id);
CREATE INDEX idx_impl_inspection_reports_type ON rsms.impl_inspection_reports(report_type_cd);
CREATE INDEX idx_impl_inspection_reports_review_date ON rsms.impl_inspection_reports(review_date);
CREATE INDEX idx_impl_inspection_reports_is_active ON rsms.impl_inspection_reports(is_active);
CREATE INDEX idx_impl_inspection_reports_plan_active ON rsms.impl_inspection_reports(impl_inspection_plan_id, is_active);
CREATE INDEX idx_impl_inspection_reports_type_active ON rsms.impl_inspection_reports(report_type_cd, is_active);
...
COMMENT ON TABLE rsms.impl_inspection_reports IS '이행점검결과보고서 테이블 - 이행점검결과보고서 정보를 관리';
COMMENT ON COLUMN rsms.impl_inspection_reports.impl_inspection_report_id IS '이행점검결과보고서ID (PK, 형식: 이행점검ID + R + 순번3자리)';
COMMENT ON COLUMN rsms.impl_inspection_reports.ledger_order_id IS '원장차수ID (FK -> ledger_order)';
COMMENT ON COLUMN rsms.impl_inspection_reports.impl_inspection_plan_id IS '이행점검ID (FK -> impl_inspection_plans)';
...
CREATE TRIGGER trg_impl_inspection_reports_updated_at
